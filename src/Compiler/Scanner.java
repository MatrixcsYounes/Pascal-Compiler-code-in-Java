...
public class Scanner {
    ...
    public Scanner(String file) throws Exception, ErreurLexicale {File file1 = new File(file);
        if (!file1.exists()) {
    ...
    public Tokens codagelex(String mot) throws IOException {
        for (Symboles motCle : this.motCles) {
            if (motCle.getNom().equals(mot)) {return motCle.getToken();}
        }
    ...
    public void symbsuiv() throws Exception {
        ...
        if (Character.isDigit(this.carCour)) {
            ...
            switch (this.carCour) {
                case '+' -> {this.symbCour = new Symboles(Tokens.PLUS_TOKEN, Character.toString(this.carCour));}
                case '-' -> {this.symbCour = new Symboles(Tokens.MOINS_TOKEN, Character.toString(this.carCour));}
                case '*' -> {this.symbCour = new Symboles(Tokens.MUL_TOKEN, Character.toString(this.carCour));}
                case '/' -> {this.symbCour = new Symboles(Tokens.DIV_TOKEN, Character.toString(this.carCour));}
                case '=' -> {this.symbCour = new Symboles(Tokens.EG_TOKEN, Character.toString(this.carCour));}
                case '(' -> {this.symbCour = new Symboles(Tokens.PARG_TOKEN, Character.toString(this.carCour));}
                case ')' -> {this.symbCour = new Symboles(Tokens.PARD_TOKEN, Character.toString(this.carCour));}
                case ',' -> {this.symbCour = new Symboles(Tokens.VIR_TOKEN, Character.toString(this.carCour));}
                case ';' -> {this.symbCour = new Symboles(Tokens.PVIR_TOKEN, Character.toString(this.carCour));}
                case '.' -> {this.symbCour = new Symboles(Tokens.PNT_TOKEN, Character.toString(this.carCour));}
                case ':' -> {lireCar();
                    if (this.carCour == '=') {
                        x = true;
                ...
                default -> {
                    this.symbCour = new Symboles(Tokens.ERR_TOKEN, CodesErr.CAR_INC_ERR.getPrivate_Message());
                }
            }
            if (!x) lireCar();
        }
