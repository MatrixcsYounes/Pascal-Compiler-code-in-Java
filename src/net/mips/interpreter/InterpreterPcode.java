...
import Compiler.ErreurSemantique;
...
public class InterpreterPcode {
    ...
    public InterpreterPcode() {
        this.pcode = new Instruction[69];
        this.pile = new int[69];
        this.pc = -1;
        this.offset = -1;
        this.sp = -1;
    }

    public void loadMnemonic(Mnemonique mne) throws ErreurSemantique {
    ...
    public void loadMnemonic(Mnemonique mne, int suite) throws ErreurSemantique {
        if (this.pc == this.pcode.length){
            throw new Error(String.valueOf(mne));
        }else {
            pc++;
